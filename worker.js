        ...
        ctx.waitUntil((async () => {
          ...
          if (!isFinished) {
            ...
            try {
              ...
              console.error('Error fetching next TG poll step:', err);
            }
          }
        })());
      } else {
  ...
  if (update.callback_query) {
    ...
    if (data === 'nav_subjects') {
      ...
      if (paperData) {
        ...
        const origin = 'https://a-l.gayanmadushanka1610.workers.dev';
        if (ctx && typeof ctx.waitUntil === 'function') {
          ctx.waitUntil(processSingleWhatsAppPollStep(paperKey, 0, 20, env));
          ctx.waitUntil(fetch(`${origin}/stream-tg-step?paperKey=${encodeURIComponent(paperKey)}&qIndex=0&intervalSec=15&sendHeader=false`));
        } else {
          processSingleWhatsAppPollStep(paperKey, 0, 20, env);
          fetch(`${origin}/stream-tg-step?paperKey=${encodeURIComponent(paperKey)}&qIndex=0&intervalSec=15&sendHeader=false`);
        }
